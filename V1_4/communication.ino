...
void Get_input(void) {
  ...
  static int out5_ = 0;
  static int out6_ = 0;
  static float out7_ = 0;
  static float out8_ = 0;
  static float out9_ = 0;

  if (Serial2.available() > 0) {

    data[i_] = Serial2.read();

    if (data[i_] == 'h' or data[i_] == 'p' or data[i_] == 's' or data[i_] == 'g' or data[i_] == 'd' or data[i_] == 'e' or data[i_] == 'o' or data[i_] == 'j' or data[i_] == 'c' or data[i_] == 'v' or data[i_] == 'i' or data[i_] == 'x') {
      flag_2 = 1;
    }
    if ( data[i_] == '\n') {
      flag_2 = 0;
    ...
    if (i_ != 0) {
      switch (data[0]) {
        ...
        ******************************************************************************/
        ...
        /*****************************************************************************
          Adjust some functional and safety features of driver.
          They are temperature limit(deg), current limit(mV) and serial data output interval(ms).
          They must be re entered on startup or else default values will be used.          
        ******************************************************************************/
        case 'i':
          data2[i2 - 1] = data[i_];
          ...
          if (data[i_]  == ',' and comma == 2) {
            ...
            data2[i2 - 1] = data[i_];
            out4_ = atoi(data2);
            short_circuit_current_reduced = out4_;
            comma = comma + 1 ;
            i2 = 0;
          }
          ...
          if (data[i_]  == '\n' and comma == 6) {
            ...
            //Output = K1 * ((position_t - Sector_offset) - EncoderCount ) + K2*(speed_t - Measured_speed)  - K3 * Measured_speed;
